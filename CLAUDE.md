...
- Защиты автозаписи (`signup.ts`): лимит в неделю (по умолчанию 2 — как у ИТМО, максимум 10); последнюю запись семестра не тратить (`have_attempts`: total — лимит ИТМО на семестр, free — остаток); не записывать при пересечении с парами
